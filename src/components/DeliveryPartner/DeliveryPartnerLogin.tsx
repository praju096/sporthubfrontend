...
import { loginDeliveryPartner } from "../../redux/features/authSlice";
...
const DeliveryPartnerLogin = () => {
  ...
  const onSubmit = async (data: LoginUser) => {
    try {
      const resultAction = await dispatch(loginDeliveryPartner(data));

      if (loginDeliveryPartner.fulfilled.match(resultAction)) {
        toast.success("Login Delivery Partner Successful");
        navigate("/welcome");
      } else {
    ...
    <div className="d-flex justify-content-center align-items-center vh-100 bg-dark text-white">
      <div className="card p-5 shadow-lg" style={{ width: '400px', borderRadius: '20px' }}>
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <h3 className="mb-4 text-center fw-bold">Delivery Partner Login</h3>
          <div className="mb-3">
