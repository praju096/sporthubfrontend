...
import { merchantLogout } from '../../redux/features/authSlice';
...
const MerchantLayout = () => {
  ...
  const merchant = useSelector((state: RootState) => state.auth.user);
  ...
  const handleMerchantLogout = () => {
    ...
    toast.info("Merchant Logged out");
  ...
  const navItems = [
    { path: '/merchant/dashboard', icon: 'fa-chart-pie', label: 'Dashboard' },
    { path: '/merchant/shop', icon: 'fa-archive', label: 'Shop' },
    { path: '/merchant/categories', icon: 'fa-copy', label: 'Categories' },
    { path: '/merchant/orders', icon: 'fa-shipping-fast', label: 'Orders' },
    { path: '/merchant/brands', icon: 'fa-tag', label: 'Brands' },
  ];
    ...
    <div className="container-fluid p-0 vh-100 bg-light position-relative">
          <header className="d-lg-none d-flex align-items-center bg-dark text-white p-3 sticky-top shadow-sm z-3">
            ...
            <h4 className="fw-bold mb-0 text-truncate">
              <i className="fas fa-building text-danger me-2"></i> SPORTHUB MERCHANT
            </h4>
          </header>
              ...
              <div className="d-flex flex-column h-100 w-100 p-3">
                <div className="d-flex align-items-center mb-4 position-relative">
                  ...
                  <div className="flex-grow-1 text-center">
                    {sidebarCollapsed ? (
                      <i className="fas fa-building text-danger fs-4"></i>
                    ) : (
                  ...
                  <button
                    className="btn btn-link text-white p-0 position-absolute end-0"
                ...
                <div className="border-top border-secondary border-bottom pt-3 pb-3 mb-3">
                  {!sidebarCollapsed ? (
                    // Expanded view
                    <div className="d-flex align-items-center gap-2">
                      <i className="bi bi-person-circle fs-1 text-secondary"></i>
                      <div className="flex-grow-1">
                        <div className="fw-bold text-truncate">{merchant?.fullname || "Admin User"}</div>
                        <button
                    ...
                    <div className="d-flex flex-column align-items-center">
                      <i className="bi bi-person-circle fs-1 text-secondary mb-2"></i>
                      <button
                  ...
                  <ul className="nav flex-column gap-2">
                    {navItems.map((item) => (
                      <li key={item.path} className="nav-item">
                        <Link
                          to={item.path}
                          className={`nav-link rounded d-flex align-items-center ${isActive(item.path)
                  ...
                  <div className="d-flex flex-column h-100 w-100">
                    ...
                    <div className="border-top border-secondary border-bottom pt-3 pb-3 d-flex align-items-center gap-2 mb-3">
                      ...
                      <div className="flex-grow-1 position-relative">
                        <div className="fw-bold text-truncate pe-4">
                          {merchant?.fullname || "Admin User"}
                        </div>
                        <button
                          className="btn btn-link text-white p-0 position-absolute top-0 end-0"
                          onClick={toggleMobileSidebar}
                        >
